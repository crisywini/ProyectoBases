...
public class InitDatabase {
	...
	public static void createAllTables() {
		...
		String createEmployeeInventaryTable = "CREATE TABLE Employee_inventary ( code_employee VARCHAR(10) NOT NULL, code_inventary int NOT NULL, PRIMARY KEY(code_employee, code_inventary), FOREIGN KEY(code_employee) REFERENCES Employee (code), FOREIGN KEY(code_inventary) REFERENCES Inventary (code) )";
		...
		String createProductSaleTable = "CREATE TABLE Product_sale ( code_product int NOT NULL, code_sale int NOT NULL, FOREIGN KEY(code_product) REFERENCES Product (code), FOREIGN KEY(code_sale) REFERENCES Sale (code) )";
		 ...
		 * entre venta y pago es de 1 a N (un medio de pago puede estar en varias
		 * ventas) + Revisar las relaciones entre cliente, cliente-servicio, servicio y
		 ...
		 * servicio puede estar en varios detalles de servicio. + La relación contrato y
		 * tipoContrato es al contrario es decir un tipo de contrato puede estar en
		 * varios contratos. Lo mismo que cargo, en un cargo pueden haber varios
		 * empleados + La tabla Domicilio es como la ruta que sea crea (supongo), no
		 * entiendo porque tiene el atributo dirección y teléfono?
		 */
	}
}
